...
// Vérifier que l'utilisateur est du rôle "secretariat"
if (!isset($_SESSION["user"]) || $_SESSION["user"]["role"] !== 'secretariat') {
    header("Location: authentification.php");
    exit();
}

// Récupérer les TDR dont l'état est "valide_directeur"valide_directeur
$stmt = $conn->prepare(
    "SELECT 
        t.*,
    ...
    WHERE etat_actuel='valide_directeur'
    GROUP BY t.idtdr
    ORDER BY t.idtdr");
...
<html>
<head>
    <title>TDR Validés par le Directeur</title>
...
<body>
    <div class="container-custom">
        <div class="header-title">📄 TDR validés par le Directeur</div>
                    ...
                    <?php else: ?>
                        ...
                        <?php foreach ($tdrs as $tdr): ?>
                            <div class="tdr-card">
                                ...
                                <a href="details_tdr_valides.php?idtdr=<?= $tdr['idtdr'] ?>" class="btn btn-view">👁 Voir les détails</a>
                            </div>
                        <?php endforeach; ?>
            <?php endif; ?>    
        </div>
