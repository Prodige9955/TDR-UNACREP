...
// Redirection si l'utilisateur n'est pas connecté ou n'est pas directeur
if (!isset($_SESSION["user"]) || $_SESSION["user"]["role"] !== 'directeur') {
    header("Location: /TDR/authentification.php");
    exit();
}

// fonction PHP pour gérer les transitions d'état afin de déterminer le nouvel état
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["tdr_id"], $_POST["decision"])) {
    ...
    $message = ($decision === "valide") 
        ? "✅ Le TDR a été validé par le directeur. Il est désormais prêt pour être transformer en Ordre de Mission au sécrétariat." 
        : "❌ Le TDR a été annulé par le directeur.";
}
?>
...
<html>
<head>
    <title>Détails du TDR - Direction</title>
    <link rel="stylesheet" href="../bootstrap/css/bootstrap.min.css">
    
    <?php if (!empty($message)) : ?>
        <meta http-equiv="refresh" content="3;URL=validation_tdr_directeur.php">
    <?php endif; ?>
</head>
                ...
                <div class="row">
                    ...
                    <form method="post">
                        ...
                        <div class="button-container text-center mt-3">
                            <button name="decision" value="valide" class="btn btn-success">✅ Valider</button>
                            <button name="decision" value="rejete" class="btn btn-danger">❌ Rejeter</button>
