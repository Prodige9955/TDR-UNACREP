...
$stmt = $conn->prepare("SELECT 
                        ...
                        WHERE etat_actuel='valide_secretariat'
                        GROUP BY t.idtdr
                        ORDER BY t.idtdr;
                        ");
...
<head>
    <meta charset="UTF-8">
    <title>Validation des TDR - Directeur</title>
...
<body>
    ...
    <div class="container-custom">
        <div class="header-title">📄 TDR en attente de validation du Directeur</div>

            <?php if (empty($tdrs)): ?>
            <div class="alert alert-info"> Aucun TDR validé par le directeur pour le moment.</div>
                <?php else: ?>
                    <?php foreach ($tdrs as $tdr): ?>
                            <div class="tdr-card">
                                ...
                                <a href="details_tdr_directeur.php?idtdr=<?= $tdr['idtdr'] ?>" class="btn btn-view">
                                    👁 Voir les détails
                                </a>
                            </div>
                    <?php endforeach; ?>
